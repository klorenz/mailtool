  ...
  MailTool = (function() {
    function MailTool(config, transport) {
      ...
      if (!config) {
        ...
        dir = this.resolveFileName("~/.mailtool");
    ...
    MailTool.prototype.hasMailbox = function(name) {
      var cfg;
      cfg = this.getConfig(name);
      return "mailbox" in cfg;
    };

    MailTool.prototype.getImapConnection = function(options) {
      var cfg, config, configFileDir, connectImap, host, logLevel, name, passwdFile, passwdFunction, port;
      if (options == null) {
        options = {};
      }
      ...
      if (name) {
        ...
        options.auth = extend({}, cfg.mailbox.auth, options.auth || {});
